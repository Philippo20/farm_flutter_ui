...
class _ModernUsersScreenState extends ConsumerState<ModernUsersScreen> {
            ...
            child: Column(
              ...
              children: [
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: Colors.white.withOpacity(0.2), borderRadius: BorderRadius.circular(AppSpacing.radiusMd)), child: const Icon(Icons.person_add, color: Colors.white, size: 24)),
                      const SizedBox(width: AppSpacing.md),
                      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('Add New User', style: AppTypography.h6.copyWith(color: Colors.white, fontWeight: FontWeight.bold)), Text('Create a new team member account', style: AppTypography.bodySmall.copyWith(color: Colors.white70))])),
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: AppSpacing.sm),
                        _buildFormTextField(controller: nameController, hint: 'e.g., John Smith', icon: Icons.person, isDark: isDark),
                        const SizedBox(height: AppSpacing.lg),
                        _buildFormLabel('Email Address', isDark),
                        const SizedBox(height: AppSpacing.sm),
                        _buildFormTextField(controller: emailController, hint: 'e.g., john@farm.com', icon: Icons.email, isDark: isDark),
                        const SizedBox(height: AppSpacing.lg),
                        if (!isMobile) Row(children: [
                          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [_buildFormLabel('Role', isDark), const SizedBox(height: AppSpacing.sm), _buildFormDropdown(value: selectedRole, items: ['Admin', 'Owner', 'Caretaker'], icon: Icons.badge, isDark: isDark, onChanged: (v) => setDialogState(() => selectedRole = v!))])),
                          const SizedBox(width: AppSpacing.md),
                          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [_buildFormLabel('Assign Farm', isDark), const SizedBox(height: AppSpacing.sm), _buildFormDropdown(value: selectedFarm, items: ['Northern Farm', 'Southern Farm', 'Eastern Farm', 'Western Farm'], icon: Icons.agriculture, isDark: isDark, onChanged: (v) => setDialogState(() => selectedFarm = v!))])),
                        ]) else ...[
                          _buildFormLabel('Role', isDark), const SizedBox(height: AppSpacing.sm), _buildFormDropdown(value: selectedRole, items: ['Admin', 'Owner', 'Caretaker'], icon: Icons.badge, isDark: isDark, onChanged: (v) => setDialogState(() => selectedRole = v!)),
                          const SizedBox(height: AppSpacing.lg),
                          _buildFormLabel('Assign Farm', isDark), const SizedBox(height: AppSpacing.sm), _buildFormDropdown(value: selectedFarm, items: ['Northern Farm', 'Southern Farm', 'Eastern Farm', 'Western Farm'], icon: Icons.agriculture, isDark: isDark, onChanged: (v) => setDialogState(() => selectedFarm = v!)),
                        ],
                      ],
                    ),
                  ),
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      const SizedBox(width: AppSpacing.md),
                      Expanded(flex: 2, child: ElevatedButton.icon(onPressed: () { Navigator.pop(context); ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Row(children: [const Icon(Icons.check_circle, color: Colors.white), const SizedBox(width: 8), Text('${nameController.text.isEmpty ? "User" : nameController.text} added!')]), backgroundColor: AppColors.success, behavior: SnackBarBehavior.floating, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(AppSpacing.radiusMd)))); }, icon: const Icon(Icons.person_add, size: 18), label: const Text('Add User'), style: ElevatedButton.styleFrom(backgroundColor: AppColors.primary, foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(vertical: AppSpacing.md), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(AppSpacing.radiusMd))))),
                    ],
                  ),
                ),
              ],
            ...
            child: Column(
              ...
              children: [
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      const SizedBox(width: AppSpacing.md),
                      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('Edit User', style: AppTypography.h6.copyWith(color: Colors.white, fontWeight: FontWeight.bold)), Text('Update user details', style: AppTypography.bodySmall.copyWith(color: Colors.white70))])),
                      IconButton(onPressed: () => Navigator.pop(context), icon: const Icon(Icons.close, color: Colors.white70)),
                    ],
                  ),
                ),
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      CircleAvatar(radius: 24, backgroundColor: _getRoleColor(user['role']).withOpacity(0.2), child: Text(user['avatar'], style: TextStyle(color: _getRoleColor(user['role']), fontWeight: FontWeight.bold))),
                      const SizedBox(width: AppSpacing.md),
                      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        Text(user['name'], style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14, color: isDark ? Colors.white : AppColors.textPrimary)),
                        Text(user['email'], style: TextStyle(fontSize: 12, color: isDark ? Colors.white60 : AppColors.textSecondary)),
                      ])),
                      Container(
                        padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                        decoration: BoxDecoration(color: _getRoleColor(user['role']).withOpacity(0.1), borderRadius: BorderRadius.circular(AppSpacing.radiusFull)),
                        child: Text(user['role'], style: TextStyle(color: _getRoleColor(user['role']), fontSize: 11, fontWeight: FontWeight.w600)),
                      ),
                    ],
                  ),
                ),
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: Column(
                      ...
                      children: [
                        _buildFormLabel('Full Name', isDark),
                        const SizedBox(height: AppSpacing.sm),
                        _buildFormTextField(controller: nameController, hint: 'Full name', icon: Icons.person, isDark: isDark),
                        const SizedBox(height: AppSpacing.lg),
                        _buildFormLabel('Email Address', isDark),
                        const SizedBox(height: AppSpacing.sm),
                        _buildFormTextField(controller: emailController, hint: 'Email', icon: Icons.email, isDark: isDark),
                        const SizedBox(height: AppSpacing.lg),
                        if (!isMobile) Row(children: [
                          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [_buildFormLabel('Role', isDark), const SizedBox(height: AppSpacing.sm), _buildFormDropdown(value: selectedRole, items: ['Admin', 'Owner', 'Caretaker'], icon: Icons.badge, isDark: isDark, onChanged: (v) => setDialogState(() => selectedRole = v!))])),
                          const SizedBox(width: AppSpacing.md),
                          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [_buildFormLabel('Status', isDark), const SizedBox(height: AppSpacing.sm), _buildFormDropdown(value: selectedStatus, items: ['Active', 'Inactive', 'Pending'], icon: Icons.toggle_on, isDark: isDark, onChanged: (v) => setDialogState(() => selectedStatus = v!))])),
                        ]) else ...[
                          _buildFormLabel('Role', isDark), const SizedBox(height: AppSpacing.sm), _buildFormDropdown(value: selectedRole, items: ['Admin', 'Owner', 'Caretaker'], icon: Icons.badge, isDark: isDark, onChanged: (v) => setDialogState(() => selectedRole = v!)),
                          const SizedBox(height: AppSpacing.lg),
                          _buildFormLabel('Status', isDark), const SizedBox(height: AppSpacing.sm), _buildFormDropdown(value: selectedStatus, items: ['Active', 'Inactive', 'Pending'], icon: Icons.toggle_on, isDark: isDark, onChanged: (v) => setDialogState(() => selectedStatus = v!)),
                        ],
                      ],
                    ),
                  ),
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      OutlinedButton(onPressed: () { Navigator.pop(context); _showDeleteUserDialog(context, user, isDark); }, style: OutlinedButton.styleFrom(padding: const EdgeInsets.symmetric(vertical: AppSpacing.md, horizontal: AppSpacing.md), side: const BorderSide(color: AppColors.error), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(AppSpacing.radiusMd))), child: const Icon(Icons.delete_outline, color: AppColors.error, size: 20)),
                      const SizedBox(width: AppSpacing.md),
                      Expanded(child: OutlinedButton(onPressed: () => Navigator.pop(context), style: OutlinedButton.styleFrom(padding: const EdgeInsets.symmetric(vertical: AppSpacing.md), side: BorderSide(color: isDark ? Colors.white24 : AppColors.neutral300), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(AppSpacing.radiusMd))), child: Text('Cancel', style: TextStyle(color: isDark ? Colors.white70 : AppColors.textSecondary)))),
                      const SizedBox(width: AppSpacing.md),
                      Expanded(flex: 2, child: ElevatedButton.icon(onPressed: () { Navigator.pop(context); ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Row(children: [const Icon(Icons.check_circle, color: Colors.white), const SizedBox(width: 8), Text('${nameController.text} updated!')]), backgroundColor: AppColors.success, behavior: SnackBarBehavior.floating, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(AppSpacing.radiusMd)))); }, icon: const Icon(Icons.save, size: 18), label: const Text('Save'), style: ElevatedButton.styleFrom(backgroundColor: AppColors.info, foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(vertical: AppSpacing.md), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(AppSpacing.radiusMd))))),
                    ],
                  ),
                ),
              ],
  ...
  void _showDeleteUserDialog(BuildContext context, Map<String, dynamic> user, bool isDark) {
    showDialog(
      context: context,
      builder: (context) => Dialog(
        ...
        child: Container(
          ...
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: AppColors.error.withOpacity(0.1), shape: BoxShape.circle), child: const Icon(Icons.person_off, color: AppColors.error, size: 40)),
              const SizedBox(height: AppSpacing.lg),
              Text('Delete User?', style: AppTypography.h5.copyWith(fontWeight: FontWeight.bold, color: isDark ? Colors.white : AppColors.textPrimary)),
              const SizedBox(height: AppSpacing.sm),
              Text('Are you sure you want to delete "${user['name']}"?', style: AppTypography.bodyMedium.copyWith(color: isDark ? Colors.white70 : AppColors.textSecondary), textAlign: TextAlign.center),
              ...
              Container(
                ...
                child: Row(
                  children: [
                    CircleAvatar(radius: 20, backgroundColor: _getRoleColor(user['role']).withOpacity(0.2), child: Text(user['avatar'], style: TextStyle(color: _getRoleColor(user['role']), fontWeight: FontWeight.bold, fontSize: 12))),
                    const SizedBox(width: AppSpacing.md),
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text(user['name'], style: TextStyle(fontWeight: FontWeight.w600, fontSize: 13, color: isDark ? Colors.white : AppColors.textPrimary)),
                      Text(user['email'], style: TextStyle(fontSize: 11, color: isDark ? Colors.white60 : AppColors.textSecondary)),
                    ])),
                  ],
                ),
              ),
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    const SizedBox(width: AppSpacing.sm),
                    Expanded(child: Text('This action cannot be undone. All user data will be permanently removed.', style: TextStyle(fontSize: 12, color: isDark ? Colors.white70 : AppColors.textSecondary))),
                  ],
                ),
              ),
              const SizedBox(height: AppSpacing.xl),
